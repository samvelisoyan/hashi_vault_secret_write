...
if state == 'present':
    ...
    if get_vault.status_code == 200:
        if data != {}:
            ...
            for key, value in data_in_vault.items():
                if key not in data:
                    final_data[key] = value

            if final_data == {}:
                vault_remove = session.delete(api_url + '/' + mount_point + '/metadata/' + secret_path)

                if vault_remove.status_code in [200,204]:
                    module.exit_json(changed=True, found=True)

                else:
                    module.fail_json(msg="faild to delete secret: " + str(vault_remove.text))

            elif final_data == data_in_vault:
                module.exit_json(changed=False, found=True)
